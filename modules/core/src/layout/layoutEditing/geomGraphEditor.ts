...
/*
  ...
  DragObjectsForRectilinearCase(delta: Point) {
    for (const node of this.objectsToDrag) {
      if (node instanceof GeomNode) node.center = node.center.add(delta)
    }
    ...
    RectilinearInteractiveEditor.CreatePortsAndRouteEdges(
      ...
      true,
      this.LayoutSettings.EdgeRoutingSettings.UseObstacleRectangles,
      this.LayoutSettings.EdgeRoutingSettings.BendPenalty,
    )
...
//  static DragEdgeWithSite(delta: Point, e: GeomEdge, site: Site) {
...
// //  <returns></returns>
//  PrepareForObjectDragging(markedObjects: IEnumerable<GeomObject>, dragMode: DraggingMode) {
...
//  static CalculateOffsetsForMultiedges(node: GeomNode, nodeSeparation: number): Dictionary<GeomEdge, number> {
...
// //  <returns></returns>
//  PrepareForPolylineCornerInsertion(affectedObj: IViewerObject, site: Site): UndoRedoAction {
