...
export class Feasibility {
  ...
  static Enforce(
    ...
    for (let level: number = settings.MinConstraintLevel; level <= currentConstraintLevel; level++) {
      ...
      const hsSolver = new AxisSolver(true, nodes, clusterHierarchies, level >= 2 && settings.AvoidOverlaps, level, clusterSettings)
      ...
      hsSolver.Solve()
      Feasibility.ResetPositions(nodes)
      const vsSolver = new AxisSolver(false, nodes, clusterHierarchies, level >= 2 && settings.AvoidOverlaps, level, clusterSettings)
