...
//  --------------------------------------------------------------------------------------------------------------------
    ...
    export class OverlapRemovalNode  {
        ...
        //  This is the normal node ctor, from ConstraintGenerator.
         constructor (id: number, userData: Object, position: number, positionP: number, size: number, sizeP: number, weight: number) {
            if ((weight <= 0)) {
                throw new Error("weight");
            }
            
            let dblCheck: number = ((Math.abs(position) + size) 
                        * weight);
            if ((!Number.isFinite(dblCheck) || Number.isNaN(dblCheck))) {
                throw new Error("position");
            }
            
            dblCheck = ((Math.abs(positionP) + sizeP) 
                        * weight);
            if ((!Number.isFinite(dblCheck) || Number.isNaN(dblCheck))) {
        ...
        //  The weight is overridden for the Cluster border nodes during Cluster.Generate.
         ...
         static Overlap(n1: OverlapRemovalNode, n2: OverlapRemovalNode, padding: number): number {
            ...
            return (((n1.Size + n2.Size) 
                        / 2) 
                        + (padding - Math.abs((n1.Position - n2.Position))));
        }
        
         static OverlapP(n1: OverlapRemovalNode, n2: OverlapRemovalNode, paddingP: number): number {
            //  Returns > 0 if the nodes overlap (combined sizes/2 plus required padding between
            //  nodes is greater than the distance between the nodes).
            return (((n1.SizeP + n2.SizeP) 
                        / 2) 
                        + (paddingP - Math.abs((n1.PositionP - n2.PositionP))));
        }
        ...
        public CreateVariable(solver: ProjectionSolver.Solver) {
            ...
            if ((this.Variable == null)) {
                this.Variable = solver.AddVariable(this, this.Position, this.Weight);
            }
            else {
        ...
        public /* override */ ToString(): string {
            return string.Format(System.Globalization.CultureInfo.InvariantCulture, "Nod '{0}': id {1} p {2:F5} s {3:F5} pP {4:F5} sP {5:F5}", this.UserDataString, this.Id, this.Position, this.Size, this.PositionP, this.SizeP);
        }
        
        public CompareTo(other: OverlapRemovalNode): number {
            ValidateArg.IsNotNull(other, "other");
            let cmp: number = this.Position.CompareTo(other.Position);
            if ((cmp == 0)) {
        ...
        public CompareTo(obj: Object): number {
            let rhs = (<OverlapRemovalNode>(obj));
            if ((rhs == null)) {
        ...
        public static Operator(lhs: Node, rhs: Node): boolean {
            if (((<Object>(lhs)) == null)) {
                //  Cast to object to avoid recursive op==
    ...
    class NodeComparer extends IComparer<OverlapRemovalNode> {
        
        public Compare(lhs: OverlapRemovalNode, rhs: OverlapRemovalNode): number {
