    ...
    export class ConstraintGenerator {
        ...
        clusterHierarchies: List<OverlapRemovalCluster> = new List<OverlapRemovalCluster>();
        ...
        ///  </summary>
        public get PaddingP(): number {
        }
        public set PaddingP(value: number)  {
        }
        ...
        public constructor (isHorizontal: boolean) : 
                this(isHorizontal, DefaultPadding, DefaultPadding) {
        ...
        public constructor (isHorizontal: boolean, padding: number, paddingP: number, clusterPadding: number, clusterPaddingP: number) {
            ...
            this.clusterHierarchies.Add(new OverlapRemovalCluster(0, null, 0, this.Padding, this.PaddingP));
        ...
        public constructor (isHorizontal: boolean, padding: number, paddingP: number) : 
                this(isHorizontal, padding, paddingP, padding, paddingP) {
        ...
        public AddNode(initialCluster: OverlapRemovalCluster, userData: Object, position: number, positionP: number, size: number, sizeP: number, weight: number): OverlapRemovalNode {
            ...
            let nodNew = new OverlapRemovalNode(this.nextNodeId++, userData, position, positionP, size, sizeP, weight);
        ...
        ///  
        public AddCluster(parentCluster: OverlapRemovalCluster, userData: Object, openBorderInfo: BorderInfo, closeBorderInfo: BorderInfo, openBorderInfoP: BorderInfo, closeBorderInfoP: BorderInfo): OverlapRemovalCluster {
        ...
        ///  <returns>The new Cluster.</returns>
        ///  
        public AddCluster(parentCluster: OverlapRemovalCluster, userData: Object, minimumSize: number, minimumSizeP: number, openBorderInfo: BorderInfo, closeBorderInfo: BorderInfo, openBorderInfoP: BorderInfo, closeBorderInfoP: BorderInfo): OverlapRemovalCluster {
            let newCluster = new OverlapRemovalCluster(this.nextNodeId, parentCluster, userData, minimumSize, minimumSizeP, this.Padding, this.PaddingP, this.ClusterPadding, this.ClusterPaddingP, openBorderInfo, closeBorderInfo, openBorderInfoP, closeBorderInfoP);
            this.nextNodeId = (this.nextNodeId + OverlapRemovalCluster.NumInternalNodes);
        ...
        public AddNodeToCluster(cluster: OverlapRemovalCluster, node: OverlapRemovalNode) {
            ...
            if ((node instanceof  OverlapRemovalCluster)) {
                throw new InvalidOperationException(#ifTEST_MSAGL, "Argument 'node' must not be a Cluster", #endif,  TEST_MSAGL);
            }
        ...
        public Generate(solver: ProjectionSolver.Solver, parameters: OverlapRemovalParameters) {
            ValidateArg.IsNotNull(solver, "solver");
            if ((parameters == null)) {
                parameters = new OverlapRemovalParameters();
            }
            
            for (let cluster: OverlapRemovalCluster in this.clusterHierarchies) {
        ...
        public Solve(solver: ProjectionSolver.Solver, parameters: OverlapRemovalParameters, doGenerate: boolean): ProjectionSolver.Solution {
            if ((solver == null)) {
            ...
            let solverSolution: ProjectionSolver.Solution = solver.Solve(parameters.SolverParameters);
            for (let cluster: OverlapRemovalCluster in this.clusterHierarchies) {
