...
test('two nodes rect', () => {
  ...
  const a = addNode(gg, 'a', CurveFactory.mkRectangleWithRoundedCorners(100, 100, 0, 0, new Point(150, 100)))

  const b = addNode(gg, 'b', CurveFactory.mkRectangleWithRoundedCorners(100, 100, 0, 0, new Point(100, 10)))

  new GeomEdge(new Edge(a, b))

  const rr = RectilinearEdgeRouter.constructorGNAN(gg, null, 1, 3)
  DebugObject.dumpDebugCurves = (fn: string, curves: DebugCurve[]) => {
    // SvgDebugWriter.dumpDebugCurves(fn, curves)
  }
  rr.run()
...
test('three nodes', () => {
  ...
  const a = addNode(gg, 'a', CurveFactory.mkRectangleWithRoundedCorners(100, 100, 0, 0, new Point(40, 0)))

  const b = addNode(gg, 'b', CurveFactory.mkRectangleWithRoundedCorners(100, 100, 1, 1, new Point(40, 80)))
