...
;(runBench ? describe : describe.skip)('Max elements rendered per tile', () => {
  ...
  afterAll(() => {
    // Plain text report
    const lines: string[] = []
    lines.push('Max elements rendered per tile (capacity ' + TILE_CAPACITY + ', no explicit Z cap; natural stops only)')
    lines.push('')
    const header =
      'Graph'.padEnd(22) +
      'Nodes'.padStart(8) +
    ...
    tex.push('\\caption{Maximum number of elements rendered in a single tile across the pyramid (capacity $\\mathcal{C}=' + TILE_CAPACITY + '$; no explicit cap on $Z$, depth determined by the natural stops of Section~\\ref{sec:tiling}).}')
